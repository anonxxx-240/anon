...
class FeedbackGraph:
    ...
    def _next_step(self, s, a, exo_info_flag, exo_info):
        ...
        if self.param_.current_problem == 'M1L':
            state_idx = s[0] if isinstance(s, (tuple, list)) else s
            action_idx = a[0] if isinstance(a, (tuple, list)) else a
            a1, a2 = self.dynamics_.sample(state_idx, action_idx, self.one_seed)
            return a1, a2, None
    ...
    def _get_richfeedback(self, s, exo_info, a = None, s_next = None, r = None):
        ...
        if self.param_.current_problem == 'M1L':
            state_idx = s[0] if isinstance(s, (tuple, list)) else s
            action_idx = a[0] if isinstance(a, (tuple, list)) else a
            return [[tuple((state_idx,action_idx)), self.dynamics_.rmax - r, s_next]]
        ...
        with tqdm(total=self.T, desc="Running", unit="step") as pbar:
            while current_t <= self.T:
                ...
                for h in range(1, self.H + 1):
                    if not self.param_.current_problem in ['M1L']:
                    ...
                    obs_set = self._get_richfeedback(s = s, exo_info = exo_info, a = a, s_next = s_next, r = r)  # returns a list of (x, r, s')
        ...
        for h in reversed(range(1, self.H + 1)):
            ...
            for s in s_set_enumerate:
                ...
                for a in self.A:
                    state_idx = s[0] if isinstance(s, (tuple, list)) else s
                    action_idx = a[0] if isinstance(a, (tuple, list)) else a
                    x = ((s,), a) if self.param_.current_problem not in ['M1L'] else tuple((state_idx, action_idx))
                    if x not in self.n:
                        continue
                ...
                state_idx = s[0] if isinstance(s, (tuple, list)) else s
                self.pi_[((state_idx,), h)] = Q_ucb[k][1]
                self.V_tilde[h - 1][(state_idx)] = Q_ucb[k][0]
                self.V_bar[h - 1][(state_idx)] = Q_lcb[k][0]
    ...
    def evaluate(self):
        ...
        with tqdm(total=self.T, desc="Running", unit="step") as pbar:
            while eval_steps < self.param_.testing_horizon:
                ...
                if self.param_.current_problem not in ['M1L']:
